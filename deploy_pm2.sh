...
if [ "${TP14_FORCE_STATE:-0}" = "1" ] || [ ! -d data/klines ] || [ -z "$(find data/klines -type f -name '*.csv' -print -quit 2>/dev/null)" ]; then
  ...
  .venv/bin/python tp14_paper_sim.py init-state --config config/paper_config.json
fi
